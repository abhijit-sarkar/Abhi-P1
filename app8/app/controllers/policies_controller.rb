class PoliciesController < ApplicationController
  ...
  def check_data
	...
	if Customer.exists?(Customer.find_by_email(params[:email]))
	  flash[:danger] = "Could not proceed.Email ID entered is in use by some existing customer."
	  exist = false
	end
	if Customer.exists?(Customer.find_by_contact(params[:contact]))
	  flash[:danger] = "Could not proceed.Contact number entered is in use by some existing customer"
	  exist = false
	end
	if (Customer.exists?(Customer.find_by_email(params[:email]))) && (Customer.exists?(Customer.find_by_contact(params[:contact])))
	  flash[:danger] = "Could not proceed.Contact number and Email ID is already in use by some other customer"
	  exist = false
	end
	if exist
	  s2 = ""			
